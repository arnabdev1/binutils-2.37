              ...
              const char *name, bfd *abfd)
{
  ...
  if (!with_symbol_versions && bfd_get_flavour(abfd) == bfd_target_elf_flavour)
  {
    atver = strchr(name, '@');
    if (atver)
      *atver = 0;
  ...
  if (info != NULL && info->elfinfo && with_symbol_versions)
  {
    ...
    version_string = bfd_get_symbol_version_string(abfd, &info->elfinfo->symbol,
                                                   false, &hidden);
               ...
               long symcount, unsigned int size)
{
  ...
  for (; from < fromend; from += size)
  {
    ...
    if (sym->name[0] == '_' && sym->name[1] == '_' && strcmp(sym->name + (sym->name[2] == '_'), "__gnu_lto_slim") == 0 && report_plugin_err)
    {
    ...
    if (undefined_only)
      ...
      keep = ((sym->flags & (BSF_GLOBAL | BSF_WEAK | BSF_GNU_UNIQUE)) != 0 || bfd_is_und_section(sym->section) || bfd_is_com_section(sym->section));
    else
      keep = 1;
    ...
    if (keep && sort_by_size && (bfd_is_abs_section(sym->section) || bfd_is_und_section(sym->section)))
      keep = 0;
    ...
    if (keep && bfd_is_target_special_symbol(abfd, sym) && !allow_special_symbols)
      keep = 0;
...
size_forward1(const void *P_x, const void *P_y)
{
  ...
  xf = (strstr(xn, "gnu_compiled") != NULL || strstr(xn, "gcc2_compiled") != NULL);
  yf = (strstr(yn, "gnu_compiled") != NULL || strstr(yn, "gcc2_compiled") != NULL);
...
#define file_symbol(s, sn, snl) \
  (((s)->flags & BSF_FILE) != 0 || ((snl) > 2 && (sn)[(snl) - 2] == '.' && ((sn)[(snl) - 1] == 'o' || (sn)[(snl) - 1] == 'a')))
                     ...
                     struct size_sym **symsizesp)
{
 ...
 bfd/section.c:global_syms like *ABS*.  */
    if ((sym->flags & (BSF_SECTION_SYM | BSF_SYNTHETIC)) == 0 && bfd_get_flavour(abfd) == bfd_target_elf_flavour)
      sz = ((elf_symbol_type *)sym)->internal_elf_sym.st_size;
    else if ((sym->flags & (BSF_SECTION_SYM | BSF_SYNTHETIC)) == 0 && bfd_is_com_section(sec))
      sz = sym->value;
    else
    {
      if (from + size < fromend && sec == bfd_asymbol_section(next))
        sz = valueof(next) - valueof(sym);
      else
        sz = (bfd_section_vma(sec) + bfd_section_size(sec) - valueof(sym));
    }
             ...
             bfd *archive_bfd)
{
  ...
  if (syminfo.type == 'i' && sym->flags & BSF_GNU_INDIRECT_FUNCTION)
  {
  ...
  if (line_numbers)
  {
    ...
    if (bfd_is_und_section(bfd_asymbol_section(sym)))
    {
      ...
      for (i = 0; i < seccount; i++)
      {
        ...
        for (j = 0; j < relcount[i]; j++)
        {
          ...
          r = relocs[i][j];
          if (r->sym_ptr_ptr != NULL && (*r->sym_ptr_ptr)->section == sym->section && (*r->sym_ptr_ptr)->value == sym->value && strcmp(symname, bfd_asymbol_name(*r->sym_ptr_ptr)) == 0 && bfd_find_nearest_line(abfd, secs[i], syms, r->address, &filename, &functionname, &lineno) && filename != NULL)
          {
    ...
    {
      if ((bfd_find_line(abfd, syms, sym, &filename, &lineno) || bfd_find_nearest_line(abfd, bfd_asymbol_section(sym),
...
set_print_width(bfd *file)
{
 ...
 is the MMO object file format.  */
    if (strstr(bfd_get_target(file), "64") != NULL || strcmp(bfd_get_target(file), "mmo") == 0)
      print_width = 64;
    else
      print_width = 32;
  }
...
int main(int argc, char **argv)
{
  ...
  while ((c = getopt_long(argc, argv, "aABCDegjlnopPrSsu",
